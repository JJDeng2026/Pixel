/**
 * 火柴人战斗 v2.1 - 修复相机/跨步/动画/武器贴图/敌人AI
 */
(function(){
'use strict';
...
const CFG={
  ...
  WEAPONS:{
    sword:{name:'剑',atkMul:1.0,range:55,speed:1.0,color:'#ffdd44',efSlash:'efSwordSlash',efSkill:'efSkillSword',wLen:22,wWid:4},
    staff:{name:'棍',atkMul:0.8,range:65,speed:1.2,color:'#ffaa44',efSlash:'efStaffImpact',efSkill:'efSkillStaff',wLen:28,wWid:3},
    knife:{name:'刀',atkMul:1.3,range:40,speed:0.85,color:'#ff6644',efSlash:'efKnifeSlash',efSkill:'efSkillKnife',wLen:18,wWid:5},
    spear:{name:'枪',atkMul:1.1,range:70,speed:0.9,color:'#88ccff',efSlash:'efSpearThrust',efSkill:'efSkillSpear',wLen:30,wWid:3},
  },
  ENEMIES:{
    sword:{name:'剑士',hp:40,atk:8,spd:1.5,atkRange:50,sight:300,atkCd:1.5,dodge:0.3,xp:20,color:'#c22',jumpChance:0.15},
    spear:{name:'枪兵',hp:35,atk:10,spd:1.8,atkRange:65,sight:320,atkCd:1.8,dodge:0.25,xp:25,color:'#c44',jumpChance:0.2},
    axe:{name:'斧兵',hp:55,atk:14,spd:1.2,atkRange:40,sight:280,atkCd:2.0,dodge:0.15,xp:35,color:'#c55',jumpChance:0.1},
  },
...
class Player{
  constructor(x,y){
    ...
    this.maxHp=CFG.PLAYER.hp;this.hp=this.maxHp;this.atk=CFG.PLAYER.atk;this.spd=CFG.PLAYER.spd;
    this.facingRight=true;this.grounded=false;
    this.atkTimer=0;this.skillTimer=0;this.invulnTimer=0;this.hitFlash=0;
    this.animState='idle';this.animTimer=0;this.walkCycle=0;
  ...
  drawStickman(ctx,ox,oy){
    ...
    if(this.animState==='walk'){const swing=Math.sin(this.walkCycle)*10;lLegX=-12+swing;rLegX=12-swing;}
    else if(this.animState==='jump'){lLegX=-8;rLegX=8;}
    else if(this.animState==='attack'){lLegX=-14;rLegX=10;}
    else if(this.animState==='skill'){lLegX=-16;rLegX=12;}
    const legEndY=oy+h*0.45;
    if(this.animState==='jump'){
      ctx.beginPath();ctx.moveTo(ox,oy+bodyLen*0.7);ctx.lineTo(ox+lLegX,legEndY-5);ctx.stroke();
      ctx.beginPath();ctx.moveTo(ox,oy+bodyLen*0.7);ctx.lineTo(ox+rLegX,legEndY-5);ctx.stroke();
    ...
    let armAngle=0.35,backArmAngle=0.35;
    if(this.animState==='walk'){const swing=Math.sin(this.walkCycle)*0.22;armAngle=0.35+swing;backArmAngle=0.35-swing;}
    else if(this.animState==='jump'){armAngle=0.55;backArmAngle=0.55;}
    else if(this.animState==='attack'){
      if(this.attackPhase<1){armAngle=0.6;backArmAngle=-0.4;}
      else if(this.attackPhase<2){armAngle=1.6;backArmAngle=-0.2;}
      else{armAngle=0.35;backArmAngle=0.35;}
    ...
    // 前臂线
    const frontArmEndX=ox+Math.cos(Math.PI*armAngle)*armLen;
    const frontArmEndY=oy-bodyLen*0.1-Math.sin(Math.PI*armAngle)*armLen;
    ctx.beginPath();ctx.moveTo(ox,oy-bodyLen*0.1);ctx.lineTo(frontArmEndX,frontArmEndY);ctx.stroke();
    // 手掌
    ctx.fillStyle='#222';ctx.beginPath();ctx.arc(frontArmEndX,frontArmEndY,3.5,0,Math.PI*2);ctx.fill();
    // 后臂线
    const backArmEndX=ox-Math.cos(Math.PI*backArmAngle)*armLen;
    const backArmEndY=oy-bodyLen*0.1-Math.sin(Math.PI*backArmAngle)*armLen;
    ctx.beginPath();ctx.moveTo(ox,oy-bodyLen*0.1);ctx.lineTo(backArmEndX,backArmEndY);ctx.stroke();
    ctx.fillStyle='#222';ctx.beginPath();ctx.arc(backArmEndX,backArmEndY,3.5,0,Math.PI*2);ctx.fill();
    // 武器常驻贴图（在攻击/技能时显示增强版，平时显示小版）
    ...
    this.drawWeaponOnHand(ctx,frontArmEndX,frontArmEndY,armAngle,weaponActive,phase);
  }
  drawWeaponOnHand(ctx,hx,hy,armAngle,active,phase){
    ...
    if(active){
      ...
      ctx.strokeStyle=isSkill?'#ff4466':wcfg.color;ctx.lineWidth=isSkill?wWid+2:wWid;
    }else{ctx.strokeStyle='#aaa';ctx.lineWidth=wWid;}
    ctx.beginPath();ctx.moveTo(midX,midY);ctx.lineTo(tipX,tipY);ctx.stroke();
    // 武器尖端
    if(active&&phase>0.3){
      ctx.fillStyle=wcfg.color.replace(')','0.6)').replace('rgb','rgba').replace('#','rgba');
      if(ctx.fillStyle.startsWith('#'))ctx.fillStyle='rgba(255,200,100,0.6)';
...
class Enemy{
  constructor(type,x,y,groundY,idx){
    ...
    this.x=x;this.y=y;this.groundY=groundY;this.maxHp=cfg.hp;this.hp=this.maxHp;
    this.atk=cfg.atk;this.spd=cfg.spd;this.atkRange=cfg.atkRange;this.sight=cfg.sight;
    this.atkCd=cfg.atkCd;this.dodge=cfg.dodge;this.jumpChance=cfg.jumpChance||0.15;
  ...
  drawStickman(ctx,ox,oy){
    ...
    let lLeg=-12,rLeg=12;
    if(this.state==='patrol'||this.state==='chase'){const swing=Math.sin(this.walkCycle)*8;lLeg=-12+swing;rLeg=12-swing;}
    else if(this.state==='attack'){lLeg=-14;rLeg=10;}
    else if(this.state==='dodge'){lLeg=-16;rLeg=12;}
    else if(this.state==='jump'||!this.grounded){lLeg=-8;rLeg=8;}
    ...
    const armLen=bodyLen*0.8;let armA=0.35,backA=0.35;
    if(this.state==='patrol'||this.state==='chase'){const s=Math.sin(this.walkCycle)*0.2;armA=0.35+s;backA=0.35-s;}
    else if(this.state==='attack'){if(this.attackPhase<1){armA=0.6;backA=-0.4;}else if(this.attackPhase<2){armA=1.6;backA=-0.2;}else{armA=0.35;backA=0.35;}}
    const fAx=ox+Math.cos(Math.PI*armA)*armLen,fAy=oy-bodyLen*0.1-Math.sin(Math.PI*armA)*armLen;
    ctx.beginPath();ctx.moveTo(ox,oy-bodyLen*0.1);ctx.lineTo(fAx,fAy);ctx.stroke();
    ctx.fillStyle='#222';ctx.beginPath();ctx.arc(fAx,fAy,3.5,0,Math.PI*2);ctx.fill();
    const bAx=ox-Math.cos(Math.PI*backA)*armLen,bAy=oy-bodyLen*0.1-Math.sin(Math.PI*backA)*armLen;
    ctx.beginPath();ctx.moveTo(ox,oy-bodyLen*0.1);ctx.lineTo(bAx,bAy);ctx.stroke();
    ctx.fillStyle='#222';ctx.beginPath();ctx.arc(bAx,bAy,3.5,0,Math.PI*2);ctx.fill();
    // 武器
    if(this.state==='attack'&&this.attackPhase>0.5){
      ctx.strokeStyle='#ff6644';ctx.lineWidth=3;
      ctx.beginPath();ctx.moveTo(fAx,fAy);ctx.lineTo(ox+Math.cos(Math.PI*armA)*armLen*1.8,oy-bodyLen*0.1-Math.sin(Math.PI*armA)*armLen*1.8);ctx.stroke();
    }
  }
  drawHealthBar(ctx,camX){
...
class Boss extends Enemy{
  constructor(type,x,y,groundY,cfgOverride){
    super('sword',x,y,groundY,0);const bcfg=cfgOverride||CFG.BOSS_CFG;
  ...
  drawBossStickman(ctx,ox,oy){
    ...
    ctx.beginPath();ctx.moveTo(ox+8,oy-bodyLen*0.3-22);ctx.lineTo(ox+14,oy-bodyLen*0.3-32);ctx.stroke();
    let lLeg=-16,rLeg=16;const swing=Math.sin(this.walkCycle)*8;
    ...
    const armLen=bodyLen*0.9;let armA=0.35;
    ...
    ctx.beginPath();ctx.moveTo(ox,oy-bodyLen*0.1);ctx.lineTo(ox-Math.cos(Math.PI*0.35)*armLen,oy-bodyLen*0.1-Math.sin(Math.PI*0.35)*armLen);ctx.stroke();
    if(this.attacking&&this.attackPhase>0.8){
      ctx.strokeStyle='#ff4444';ctx.lineWidth=6;
      ctx.beginPath();ctx.moveTo(ox,oy-bodyLen*0.1);ctx.lineTo(ox+Math.cos(Math.PI*armA)*armLen*2,oy-bodyLen*0.1-Math.sin(Math.PI*armA)*armLen*2);ctx.stroke();
    }
  }
...
class Game{
  constructor(){
    ...
    this.sceneIdx=DB.data.scene-1;this.sceneCfg=CFG.SCENES[this.sceneIdx];
    this.worldW=this.sceneCfg.width;this.groundY=this.gameH*0.82;this.camX=0;
    this.player=new Player(150,this.groundY-60);this.player.weapon=DB.data.weapon;
  ...
  initScene(){
    this.enemies=[];this.boss=null;this.sceneClear=false;this.transitionTimer=0;
    const cfg=this.sceneCfg;this.bgRenderer=new BackgroundRenderer(cfg);
    const spawnArea=this.worldW*0.4;
    if(cfg.boss){
      this.boss=new Boss('sword',spawnArea+rand(0,200),this.groundY-40,this.groundY,{hp:CFG.BOSS_CFG.hp+(this.sceneIdx>=7?200:0),armor:CFG.BOSS_CFG.armor+(this.sceneIdx>=7?100:0),atk:CFG.BOSS_CFG.atk+(this.sceneIdx>=7?10:0),spd:CFG.BOSS_CFG.spd,atkRange:CFG.BOSS_CFG.atkRange,sight:CFG.BOSS_CFG.sight,atkCd:CFG.BOSS_CFG.atkCd,stunDuration:CFG.BOSS_CFG.stunDuration,xp:CFG.BOSS_CFG.xp,bossName:cfg.bossName||'BOSS',finalBoss:cfg.finalBoss||false});
    }else{let idx=0;for(const group of cfg.enemies){for(let i=0;i<group.count;i++){this.enemies.push(new Enemy(group.type,spawnArea+rand(i*120,i*120+100),this.groundY-29,this.groundY,idx++));}}}
    this.player.x=100;this.player.y=this.groundY-30;this.player.vx=0;this.player.vy=0;this.player.grounded=true;this.camX=0;this.player.hp=this.player.maxHp;this.player.weapon=DB.data.weapon;
  }
  setupInput(){
    this.canvas.addEventListener('touchstart',e=>this.onTouchStart(e),{passive:false});this.canvas.addEventListener('touchmove',e=>this.onTouchMove(e),{passive:false});this.canvas.addEventListener('touchend',e=>this.onTouchEnd(e),{passive:false});this.canvas.addEventListener('touchcancel',e=>this.onTouchEnd(e),{passive:false});
    this.canvas.addEventListener('mousedown',e=>this.onMouseDown(e));window.addEventListener('mousemove',e=>this.onMouseMove(e));window.addEventListener('mouseup',e=>this.onMouseUp(e));
    window.addEventListener('keydown',e=>this.onKeyDown(e));window.addEventListener('keyup',e=>this.onKeyUp(e));window.addEventListener('resize',()=>this.resize());
  }
  ...
  onTouchStart(e){e.preventDefault();for(const t of e.changedTouches){const p=this.screenToGame(t.clientX,t.clientY);this.activeTouches[t.identifier]=p;if(this.state==='gameover'||this.state==='victory'){this.handleClick(p.x,p.y);continue;}if(this.joystick.handleDown(p.x,p.y,t.identifier))continue;const btn=this.checkButton(p.x,p.y);if(btn){this.touchButtonMap[t.identifier]=btn;this.handleButton(btn);}}}
  onTouchMove(e){e.preventDefault();for(const t of e.changedTouches){const p=this.screenToGame(t.clientX,t.clientY);this.activeTouches[t.identifier]=p;this.joystick.handleMove(p.x,p.y,t.identifier);}}
  onTouchEnd(e){for(const t of e.changedTouches){this.joystick.handleUp(t.identifier);delete this.activeTouches[t.identifier];delete this.touchButtonMap[t.identifier];}}
  onMouseDown(e){const p=this.screenToGame(e.clientX,e.clientY);if(this.state==='gameover'||this.state==='victory'){this.handleClick(p.x,p.y);return;}if(this.joystick.handleDown(p.x,p.y,'mouse'))return;const btn=this.checkButton(p.x,p.y);if(btn)this.handleButton(btn);}
  ...
  onKeyDown(e){switch(e.key){case'ArrowLeft':case'a':this.joystick.dx=-1;break;case'ArrowRight':case'd':this.joystick.dx=1;break;case'ArrowUp':case'w':this.joystick.dy=-1;break;case'ArrowDown':case's':this.joystick.dy=1;break;case'j':case'J':this.handleButton('attack');break;case'k':case'K':this.handleButton('jump');break;case'l':case'L':this.handleButton('skill');break;}}
  onKeyUp(e){switch(e.key){case'ArrowLeft':case'a':case'ArrowRight':case'd':if(!e.ctrlKey&&!e.metaKey)this.joystick.dx=0;break;case'ArrowUp':case'w':case'ArrowDown':case's':if(!e.ctrlKey&&!e.metaKey)this.joystick.dy=0;break;}}
  ...
  checkHit(hit){for(const enemy of this.enemies){if(enemy.dead)continue;if(this.hitOverlap(hit,enemy)){const killed=enemy.takeDamage(hit.damage,hit.knockback);this.spawnHitParticles(enemy.x,enemy.y,hit.color);if(killed)this.spawnDeathParticles(enemy.x,enemy.y);}}if(this.boss&&!this.boss.dead){if(this.hitOverlap(hit,this.boss)){const killed=this.boss.takeDamage(hit.damage,hit.knockback);this.spawnHitParticles(this.boss.x,this.boss.y,hit.color);if(killed)this.spawnDeathParticles(this.boss.x,this.boss.y);}}}
  ...
  update(dt){
    ...
    // 场景切换倒计时
    if(this.transitionTimer>0){this.transitionTimer-=dt;if(this.transitionTimer<=0){this.doSceneTransition();}}
    if(this.player.hp<=0){this.state='gameover';this.gameOverCause='被敌人击败';}
    if(!this.player.grounded)this.joystick.jumpPressed=false;
  }
  doSceneTransition(){
    this.sceneIdx++;if(this.sceneIdx>=CFG.SCENES.length){this.state='victory';this.victoryScene=true;return;}
    this.sceneCfg=CFG.SCENES[this.sceneIdx];this.worldW=this.sceneCfg.width;DB.data.scene=this.sceneIdx+1;
    if(this.sceneIdx+1>DB.data.maxScene)DB.data.maxScene=this.sceneIdx+1;DB.save();this.initScene();
  }
  render(){
    ...
    if(this.camX>=this.worldW-w-5){const sc=this.sceneClear||this.sceneCfg.boss;ctx.fillStyle=sc?'rgba(255,255,255,0.3)':'rgba(255,100,100,0.5)';ctx.fillText(sc?'▶ 进入下一关':'▶ 击败敌人',w-40,h/2);}
    ctx.textAlign='start';
    ...
    if(this.transitionTimer>0){ctx.fillStyle='rgba(255,204,0,0.8)';ctx.font='bold 18px sans-serif';ctx.textAlign='center';ctx.fillText('即将进入下一关...',w/2,h/2-40);ctx.textAlign='start';}
    if(this.state==='gameover'){
      ctx.fillStyle='rgba(0,0,0,0.7)';ctx.fillRect(0,0,w,h);ctx.fillStyle='#ff4444';ctx.font='bold 28px sans-serif';ctx.textAlign='center';ctx.fillText('战斗失败',w/2,h/2-30);
      ctx.fillStyle='#fff';ctx.font='16px sans-serif';ctx.fillText(this.gameOverCause,w/2,h/2+10);ctx.fillStyle='#ffcc00';ctx.font='14px sans-serif';ctx.fillText('点击重新开始',w/2,h/2+50);ctx.textAlign='start';
    }
    if(this.state==='victory'){
      ctx.fillStyle='rgba(0,0,0,0.7)';ctx.fillRect(0,0,w,h);ctx.fillStyle='#ffcc00';ctx.font='bold 28px sans-serif';ctx.textAlign='center';ctx.fillText('恭喜通关!',w/2,h/2-20);
      ctx.fillStyle='#fff';ctx.font='16px sans-serif';ctx.fillText('所有敌人已被击败',w/2,h/2+20);ctx.fillStyle='#ffcc00';ctx.font='14px sans-serif';ctx.fillText('点击返回主菜单',w/2,h/2+60);ctx.textAlign='start';
    }
  }
  drawButtons(ctx){
  ...
  restart(){this.sceneIdx=DB.data.scene-1;this.sceneCfg=CFG.SCENES[this.sceneIdx];this.worldW=this.sceneCfg.width;this.player.hp=this.player.maxHp;this.state='playing';this.gameOverCause='';this.victoryScene=false;this.initScene();}
...
function updateMenuDisplay(){
  ...
  const wg=document.getElementById('weaponGrid');if(wg){wg.innerHTML='';for(const k in CFG.WEAPONS){const wcfg=CFG.WEAPONS[k];const div=document.createElement('div');div.className='weapon-btn'+(DB.data.weapon===k?' selected':'');div.textContent=wcfg.name;div.onclick=()=>{DB.data.weapon=k;DB.save();updateMenuDisplay();};wg.appendChild(div);}}
  const sg=document.getElementById('sceneGrid');if(sg){sg.innerHTML='';for(let i=0;i<CFG.SCENES.length;i++){const scene=CFG.SCENES[i];const locked=i>=DB.data.maxScene;const div=document.createElement('div');div.className='scene-btn'+(locked?' locked':'');div.innerHTML='<span class="sc-num">'+(i+1)+'</span><span class="sc-name">'+(scene.boss?'BOSS':scene.name)+'</span>';if(!locked){div.onclick=()=>{DB.data.scene=i+1;DB.save();document.getElementById('mainMenu').classList.add('hidden');document.getElementById('gameScreen').classList.remove('hidden');new Game();};}sg.appendChild(div);}}
}

// ==================== 启动 ====================
let _game=null;
function startGame(){const lo=document.getElementById('loadOverlay');if(lo)lo.style.display='none';try{document.getElementById('mainMenu').classList.add('hidden');document.getElementById('gameScreen').classList.remove('hidden');if(_game){_game.running=false;}_game=new Game();}catch(e){console.error('startGame error:',e&&e.message?e.message:e,e&&e.stack?e.stack:'');}}
